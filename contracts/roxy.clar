...
;; User Staking System
(define-map yes-stakes (tuple (event-id uint) (user principal)) uint)
(define-map no-stakes (tuple (event-id uint) (user principal)) uint)
...
(define-map transaction-logs uint {
...
(define-map guild-members (tuple (guild-id uint) (user principal)) bool) ;; Is user a member of guild
(define-map guild-deposits (tuple (guild-id uint) (user principal)) uint) ;; User's contribution to guild pool
(define-map guild-yes-stakes (tuple (guild-id uint) (event-id uint)) uint) ;; Guild's YES stake on event
(define-map guild-no-stakes (tuple (guild-id uint) (event-id uint)) uint) ;; Guild's NO stake on event
            ...
            event (begin
                    ...
                    (match (map-get? user-points user)
                        current-points (begin
                                ...
                                ;; Add to YES pool
                                (let ((new-yes-pool (+ (get yes-pool event) amount))
                                    ...
                                    (match (map-get? yes-stakes (tuple (event-id event-id) (user user)))
                                        existing-stake (map-set yes-stakes (tuple (event-id event-id) (user user)) (+ existing-stake amount))
                                        (begin
                                            (map-set yes-stakes (tuple (event-id event-id) (user user)) amount)
            ...
            event (begin
                    ...
                    (match (map-get? user-points user)
                        current-points (begin
                                ...
                                (map-set user-points user (- current-points amount))
                                ;; Add to NO pool
                                (let ((new-yes-pool (get yes-pool event))
                                    ...
                                    (match (map-get? no-stakes (tuple (event-id event-id) (user user)))
                                        existing-stake (map-set no-stakes (tuple (event-id event-id) (user user)) (+ existing-stake amount))
                                        (begin
                                            (map-set no-stakes (tuple (event-id event-id) (user user)) amount)
                                            ;; Track new prediction for leaderboard
                                            (match (map-get? user-stats user)
                                                stats (map-set user-stats user {
                                                    total-predictions: (+ (get total-predictions stats) u1),
                    ...
                    (let ((yes-pool (get yes-pool event))
                        ...
                        (let ((log-id (var-get next-log-id))
                            ...
                            (map-set transaction-logs log-id {
                                ...
                                listing-id: none,
                                amount: none,
                                metadata: (if winner "winner-yes" "winner-no")
                            })
                            (var-set next-log-id (+ log-id u1))
                        )
                        (ok true)
                                                            ...
                                                            (if (> stake u0)
                                                                (let ((reward (/ (* stake total-pool) winning-pool)))
                                                                    ...
                                                                    (let ((new-earned-points (+ current-earned reward)))
                                                                        (map-set earned-points user new-earned-points)
                                                                        ;; Update total YES stakes (subtract before clearing)
                                                                        (var-set total-yes-stakes (- (var-get total-yes-stakes) stake))
                                                                        ;; Clear the stake
                                                                        (map-set yes-stakes (tuple (event-id event-id) (user user)) u0)
                                                                                                    ;; Update leaderboard stats (WIN)
                                                                                                    (match (map-get? user-stats user)
                                                                                                        stats (begin
                                                                                                            (let ((new-wins (+ (get wins stats) u1))
                                                                                                                  (new-total-earned (+ (get total-points-earned stats) reward))
                                                                                                                  (new-win-rate (/ (* new-wins u10000) (+ new-wins (get losses stats)))))
                                                                                                                (map-set user-stats user {
                                                                        ...
                                                                        no-stake (begin
                                                                            (if (> no-stake u0)
                                                                                (begin
                                                                                    ...
                                                                                    (var-set total-no-stakes (- (var-get total-no-stakes) no-stake))
                                                                                    (map-set no-stakes (tuple (event-id event-id) (user user)) u0)
                                                                                    ...
                                                                                    (match (map-get? earned-points user)
                                                                                        current-earned (begin
                                                                                                (let ((new-earned-points (+ current-earned reward)))
                                                                                                    (map-set earned-points user new-earned-points)
                                                                                                    ;; Update total NO stakes (subtract before clearing)
                                                                                                    (var-set total-no-stakes (- (var-get total-no-stakes) stake))
                                                                                                    ;; Clear the stake
                                                                                                    (map-set no-stakes (tuple (event-id event-id) (user user)) u0)
                                                                                                    ;; Update leaderboard stats (WIN)
                                                                                                    (match (map-get? user-stats user)
                    ...
                    (let ((seller (get seller listing))
                        ...
                        (let ((price-per-point (/ total-price-stx total-points))
                            ...
                            (match (map-get? user-points buyer)
                                buyer-points (map-set user-points buyer (+ buyer-points points-to-buy))
    ...
    (let ((creator tx-sender))
        (match (map-get? guilds guild-id)
            ...
            (begin
                ...
                (map-set guild-members (tuple (guild-id guild-id) (user creator)) true)
                (map-set guild-deposits (tuple (guild-id guild-id) (user creator)) u0)
                ;; Emit event
                (print {
                    event: "guild-created",
                    guild-id: guild-id,
                                ...
                                current-points (begin
                                    ...
                                    (match (map-get? guild-deposits (tuple (guild-id guild-id) (user user)))
                                        existing-deposit (begin
                                            (map-set guild-deposits (tuple (guild-id guild-id) (user user)) (+ existing-deposit amount))
                                            (map-set guilds guild-id {
                                                creator: (get creator guild),
                                                name: (get name guild),
                                        ...
                                        (begin
                                            (map-set guild-deposits (tuple (guild-id guild-id) (user user)) amount)
                                            (map-set guilds guild-id {
                                                creator: (get creator guild),
                                                name: (get name guild),
                                                total-points: (+ (get total-points guild) amount),
                        ...
                        (begin
                            (match (map-get? guild-deposits (tuple (guild-id guild-id) (user user)))
                                user-deposit (begin
                                    ...
                                    (map-set guild-deposits (tuple (guild-id guild-id) (user user)) (- user-deposit amount))
                                ...
                                event (begin
                                    ...
                                    (let ((new-yes-pool (+ (get yes-pool event) amount))
                                        ...
                                        (match (map-get? guild-yes-stakes (tuple (guild-id guild-id) (event-id event-id)))
                                            existing-stake (map-set guild-yes-stakes (tuple (guild-id guild-id) (event-id event-id)) (+ existing-stake amount))
                                            (begin
                                                (map-set guild-yes-stakes (tuple (guild-id guild-id) (event-id event-id)) amount)
                                        ...
                                        (var-set total-guild-yes-stakes (+ (var-get total-guild-yes-stakes) amount))
                                        ;; Emit event
                                        (print {
                                ...
                                event (begin
                                    ...
                                    (let ((new-yes-pool (get yes-pool event))
                                        ...
                                        (match (map-get? guild-no-stakes (tuple (guild-id guild-id) (event-id event-id)))
                                            existing-stake (map-set guild-no-stakes (tuple (guild-id guild-id) (event-id event-id)) (+ existing-stake amount))
                                            (begin
                                                (map-set guild-no-stakes (tuple (guild-id guild-id) (event-id event-id)) amount)
                                                ;; Track new prediction for guild leaderboard
                                                (match (map-get? guild-stats guild-id)
                                                    stats (map-set guild-stats guild-id {
                                                        total-predictions: (+ (get total-predictions stats) u1),
                                        ...
                                        (var-set total-guild-no-stakes (+ (var-get total-guild-no-stakes) amount))
                                        ;; Emit event
                                        (print {
                                                                ...
                                                                stake (begin
                                                                    (if (> stake u0)
                                                                        (let ((reward (/ (* stake total-pool) winning-pool)))
                                                                            ...
                                                                            (var-set total-guild-yes-stakes (- (var-get total-guild-yes-stakes) stake))
                                                                            ;; Clear the stake
                                                                ...
                                                                stake (begin
                                                                    (if (> stake u0)
                                                                        (let ((reward (/ (* stake total-pool) winning-pool)))
                                                                            ...
                                                                            (var-set total-guild-no-stakes (- (var-get total-guild-no-stakes) stake))
                                                                            ;; Clear the stake
...
;; ============================================================================
(define-read-only (get-yes-stake (event-id uint) (user principal))
    (map-get? yes-stakes (tuple (event-id event-id) (user user)))
)
...
;; ============================================================================
(define-read-only (get-no-stake (event-id uint) (user principal))
    (map-get? no-stakes (tuple (event-id event-id) (user user)))
)
...
(define-read-only (is-guild-member (guild-id uint) (user principal))
    (map-get? guild-members (tuple (guild-id guild-id) (user user)))
)
...
;; ============================================================================
(define-read-only (get-guild-deposit (guild-id uint) (user principal))
    (map-get? guild-deposits (tuple (guild-id guild-id) (user user)))
)
...
;; ============================================================================
(define-read-only (get-guild-yes-stake (guild-id uint) (event-id uint))
    (map-get? guild-yes-stakes (tuple (guild-id guild-id) (event-id event-id)))
)
...
;; ============================================================================
(define-read-only (get-guild-no-stake (guild-id uint) (event-id uint))
    (map-get? guild-no-stakes (tuple (guild-id guild-id) (event-id event-id)))
)
...
(define-private (is-member? (guild-id uint) (user principal))
    (match (map-get? guild-members (tuple (guild-id guild-id) (user user)))
...
;;   - (ok true) on success
;; ============================================================================
(define-private (increase-points (user principal) (amount uint))
    (match (map-get? user-points user)
        current-points (begin
                (map-set user-points user (+ current-points amount))
                (ok true)
